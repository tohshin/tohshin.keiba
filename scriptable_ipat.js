...
async function main() {
  ...
  (function() {
    ...
    function setDiag(m, col) {
      ...
      if (!x) {
        ...
        x.id = "ipat-diag";
        x.style = "position:fixed;top:0;left:0;width:100%;z-index:100000;background:" + (col || "rgba(79,70,229,0.95)") + ";color:#ffffff;font-size:12px;font-weight:bold;padding:8px 12px;pointer-events:none;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,0.3);";
  ...
  for (let waitCount = 0; waitCount < 30; waitCount++) {
    ...
    if (status === "LOGIN_ERROR") {
      let errA = new Alert();
      errA.title = "即PATログインエラー";
      errA.message = "加入者番号、暗証番号、またはP-ARS番号に誤りがあります。\nKeychainの情報を再設定してください。";
      errA.addAction("設定を再入力する");
      errA.addCancelAction("キャンセル");
      let choice = await errA.present();
      if (choice === 0) {
        Keychain.remove("ipat_user_no");
        Keychain.remove("ipat_pass_no");
        Keychain.remove("ipat_pars_no");
      }
      Script.complete();
      return;
    }
    ...
    (function() {
      function setDiag(m) {
        ...
        if (!x) {
          x = document.createElement("div");
          x.id = "ipat-diag";
          x.style = "position:fixed;top:0;left:0;width:100%;z-index:100000;background:rgba(79,70,229,0.95);color:#ffffff;font-size:12px;font-weight:bold;padding:8px 12px;pointer-events:none;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,0.3);";
          document.body.appendChild(x);
        }
        x.innerText = m;
      }
      ...
      try {
        ...
        // Step 4: 方式選択 (通常/ボックス/ながし)
          ...
          if (!isSimple && hasHouScreen) {
            ...
            // 方式画面が存在しない、または単勝/複勝の場合はスキップして馬番選択へ
            step = 5;
            retryCount = 0;
            setTimeout(stepLoop, 200);
            return;

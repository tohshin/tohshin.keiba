    ...
    <script>
        ...
        async function fetchRaceResults(raceId) {{
            ...
            try {{
                ...
                // allorigins の raw エンドポイントを使用
                const proxyUrl = `https://api.allorigins.win/raw?url=${{encodeURIComponent(targetUrl)}}&t=${{new Date().getTime()}}`;
                
                console.log("[DEBUG] Fetching results via AllOrigins Raw:", proxyUrl);
                ...
                if (payoutTables.length === 0) {{
                    resultDiv.innerHTML = '<div style="color:var(--text-muted); text-align:center;">Results not available yet.</div>';
                    return;
                }}
                ...
                payoutTables.forEach(table => {{
                    resultsHtml += `<div style="font-size:0.8rem; margin-bottom:10px; overflow-x:auto;">${{table.outerHTML}}</div>`;
                }});
                ...
                html += `
                    <div class="strategy-item-modal">
                        ...
                        <div style="font-size: 0.75rem; color: var(--text-muted); text-align: right; margin-top: 8px;">
                            Z-Score: Jiku1 > ${{s.score_th}} ${{s.axis_count >= 2 ? ` / Jiku2 > ${{s.axis2_score_th || s.partner_score_th}}` : ''}} / Partner > ${{s.partner_score_th}}
                        </div>
                    </div>
                `;
            }});

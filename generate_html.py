...
<body>
    ...
    <script>
                ...
                html += `
                    <div class="strategy-item-modal" data-strategy-type="${{s.type}}">
                        ...
                        <div style="margin-top: 8px; text-align: right;">
                            <button class="smappy-btn" data-eyes="${{bettingEyes}}" data-type="${{s.type}}" data-round="${{raceData.round}}" data-axis="${{s.axis_count || 1}}" onclick="event.stopPropagation(); showSmappy(this)">📌 スマッピー</button>
                        </div>
                    </div>
                `;
            }});
        ...
        function showSmappy(btn) {{
            ...
            // 会場の判定ロジック
            var raceId = Object.keys(currentData).find(k => currentData[k].round == round && currentData[k].horses.some(h => eyes.includes(h.horse_name) || eyes.includes(String(h.horse_number).padStart(2, '0'))));
            // 実際はもっとシンプルに、現在表示中の会場名をPLACE_DICT_CHUOHでソートしてインデックスを決める
            var currentPlace = "";
            if (raceId) currentPlace = currentData[raceId].place;
            ...
            popup.innerHTML = `
                <div class="smappy-tabs">
                    <div id="tab-pc" class="smappy-tab active" onclick="switchSmappyTab('pc')">💻 PC / Android</div>
                    <div id="tab-ios" class="smappy-tab" onclick="switchSmappyTab('ios')">🍎 iPhone (Shortcuts)</div>
                </div>
                ...
                <div id="panel-ios" style="display: none;">
                    ...
                    <div class="step-box">
                        ...
                        <div class="step-desc">下のボタンで「暗号」をコピーして、JRAでそのショートカットを呼ぶだけ！</div>
                    </div>
                    <button onclick="copySmappyShortcutJS()" style="width: 100%; padding: 12px; background: #10b981; color: #fff; border: none; border-radius: 8px; font-weight: 800; font-size: 0.85rem; cursor: pointer; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);">🍎 実行用の暗号をコピー</button>
                </div>
        ...
        function genSmappyShortcutJS(venue, raceRound, siki, hou, axes, partners) {{
            var raceVal = String(parseInt(raceRound) - 1);
            var simple = (siki === '1' || siki === '2' || siki === '9');
            var steps = "'" + venue + "','" + raceVal + "','" + siki + "'";
            if (!simple) steps += ",'" + hou + "'";
        ...
        function copySmappyShortcutJS() {{
            ...
            var v = venueEl.value;
            var p = window._smappyParsed;
            var js = genSmappyShortcutJS(v, p.round, p.siki, p.hou, p.axes, p.partners);
            var t = document.createElement('textarea');
            t.value = js;
            document.body.appendChild(t);
            t.select();
            document.execCommand('copy');
            document.body.removeChild(t);
            alert('iOSショートカット用JSをコピーしました！\n\nショートカットアプリ→WebページでJavaScriptを実行→貼り付け');
        }}

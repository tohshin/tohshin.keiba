    ...
    <script>
                    ...
                    horsesHtml += `
                        <div class="horse-row ${{rankClass}} ${{highlightClass}}" style="flex-wrap: wrap; padding-left: 8px;">
                            <div style="display: flex; width: 100%; align-items: center; margin-bottom: 6px;">
                                ...
                                <div class="horse-score-val" style="margin-left: 14px;" title="${{mainModelKey}} Z-Score">${{getZ(horse, mainModelKey).toFixed(4)}}</div>
                            </div>
            ...
            if (!hasValidRec) {{
                html += `
                    <div style="padding: 40px 20px; text-align: center; background: rgba(255,255,255,0.02); border-radius: 12px; border: 1px dashed rgba(255,255,255,0.1); color: var(--text-muted); margin-bottom: 20px;">
                        ...
                        <div style="font-weight: 600; font-size: 0.8rem; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.05em;">Currently no recommendations available</div>
                        <div style="font-weight: 800; color: #fff; font-size: 1.1rem;">オススメの買い目はありません</div>
                    </div>
                `;
            }}
        ...
        async function fetchRaceResults(raceId) {{
            ...
            try {{
                ...
                let htmlRes = `
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                        <span style="color:#4ade80; font-weight:800; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em;">Confirmed Results</span>
                        <span style="font-size:0.7rem; color:var(--text-muted); font-weight:600;">${{doc.title.split('|')[0].trim()}}</span>
                    </div>
                    <div style="display: flex; flex-direction: column; gap: 8px;">
                `;

                rows.forEach(rowTypes => {{
                    const availableInRow = Object.keys(payoutData.nums).filter(t => rowTypes.some(rt => t.includes(rt)));
                    if (availableInRow.length > 0) {{
                        htmlRes += '<div style="display: flex; gap: 6px; flex-wrap: nowrap; overflow-x: auto; padding-bottom: 2px;">';
                        availableInRow.forEach(type => {{
                            const nums = payoutData.nums[type];
                            ...
                            if (type === 'ワイド' && nums.length >= 2) {{
                                ...
                                numbersHtml = pairs.map(p => `<span style="color:#4ade80; font-size:0.75rem; font-weight:900;">${{p}}</span>`).join('<span style="color:var(--text-muted); font-size:0.6rem; font-weight:bold; margin:0 1px;">,</span>');
                            }} else {{
                                numbersHtml = nums.map((n, i) => `
                                    <span style="color:#4ade80; font-size:0.75rem; font-weight:900;">${{n}}</span>
                                    ${{i < nums.length - 1 ? `<span style="color:var(--text-muted); font-size:0.65rem; font-weight:bold;">${{separator}}</span>` : ''}}
                                `).join('');
                            }}

                            htmlRes += `
                                <div style="background:rgba(255,255,255,0.04); padding:6px 10px; border-radius:8px; border:1px solid rgba(255,255,255,0.08); flex: 1; min-width: fit-content;">
                                    <div style="font-size:0.6rem; color:var(--text-muted); margin-bottom:2px; font-weight:700;">${{displayType}}</div>
        ...
        function adjustFontSize() {{
            const items = document.querySelectorAll('.strategy-item-modal');
            items.forEach(item => {{

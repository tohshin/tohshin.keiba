    ...
    <script>
        ...
        async function checkAuth() {{
            ...
            try {{
                ...
                console.log("Password length entered:", pw.length);
        ...
        window.onload = function() {{
            ...
            try {{
                ...
                if (authTime) {{
                    ...
                    console.log("Auth time diff (hours):", diffHours);
                    if (diffHours < 24) {{
                        console.log("Automatically logging in from valid session");
                        document.getElementById('auth-overlay').style.display = 'none';
                        document.getElementById('app-content').style.display = 'block';
                        loadData();
                        return;
                ...
                console.log("Relogging required, showing overlay");
        ...
        function checkHits(payoutData) {{
            ...
            strategyItems.forEach(item => {{
                ...
                const container = item.querySelector('.bet-eyes-text-container');
                if (eyesText && container) {{
                    let fontSize = 1.8; // rem
                    eyesText.style.fontSize = fontSize + 'rem';
                    while (eyesText.scrollWidth > container.offsetWidth && fontSize > 0.6) {{
                        fontSize -= 0.1;
                        eyesText.style.fontSize = fontSize + 'rem';
                    }}
                }}

...
<body>
    ...
    <script>
            ...
            strategyItems.forEach(item => {{
                ...
                if (resultArea) {{
                    resultArea.innerHTML = `
                        <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; font-size: 0.75rem;">
                            <div><span style="color:var(--text-muted);">Bets:</span> <span style="color:#fff; font-weight:700;">${{eyesCount}}</span></div>
                            <div><span style="color:var(--text-muted);">Payout:</span> <span style="color:#fff; font-weight:700;">${{totalPay.toLocaleString()}}円</span></div>
                            <div><span style="color:var(--text-muted);">Profit:</span> <span style="color:${{profit >= 0 ? '#4ade80' : '#ef4444'}}; font-weight:800;">${{profit >= 0 ? '+' : ''}}${{profit.toLocaleString()}}円</span></div>
                        </div>
                    `;
                }}
                ...
                if (isHit) {{
                    ...
                    hitBadge.style.cssText = 'position:absolute; top:12px; right:12px; background:#4ade80; color:#064e3b; font-size:0.55rem; font-weight:900; padding:1px 4px; border-radius:8px; box-shadow:0 1px 4px rgba(0,0,0,0.3); z-index:10;';
        ...
        function showRecommendation(raceId) {{
            ...
            raceData.strategies.forEach(s => {{
                // Need to pass stats for Z-Score filtering in generateBettingEyes
                const scoreModels = ['LightGBM_raw', 'XGBoost_raw', 'CatBoost_raw', 'LSTM_raw', 'RandomForest_raw', 'DecisionTree_raw', 'Ensemble'];
                const stats = {{}};
                scoreModels.forEach(m => {{
                    const vals = raceData.horses.map(h => parseFloat(h[m]) || 0);
